...
//                ExifInterface.TAG_PHOTOGRAPHIC_SENSITIVITY,
        ...
        try {
            ...
            for (String attribute : attributes) {
                value = newExif.getAttribute(attribute);
    ...
    public static String saveBitmap(Bitmap b) {
        String jpegName = Utils.getApp().getCacheDir().getAbsolutePath() + File.separator + "picture_" + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(new Date()) + ".jpg";
        try {
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.ECLAIR)
    public static String compressPicture(String path, int reqWidth, int reqHeiht, int quality) {
        ...
        try {
            ...
            Log.d("BitmapUtil", "图片的原有方向：" + orientation);
        } catch (IOException e) {
            e.printStackTrace();
            Log.d("BitmapUtil", "图片的原有方向：" + "垃圾");
        }
        BitmapUtil.log(path);
        Bitmap originBitmap = BitmapFactory.decodeFile(path);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        originBitmap.compress(Bitmap.CompressFormat.JPEG, 100, bos);
        Bitmap bitmap = compressBitmap(bos.toByteArray(), reqWidth, reqHeiht, quality);

        File dirFile = new File(Utils.getApp().getFilesDir() + "/compress/");
        if (!dirFile.exists()) {
            dirFile.mkdirs();
        }
        File imgFile = new File(dirFile, "compress" + sys + ".jpg");
        try {
            ...
            try {
                ExifInterface newExif = new ExifInterface(absolutePath);
     ...
     * @return
     */
    public static Bitmap compressBitmap(byte[] data, int reqWidth, int reqHeight, int quality) {
        Log.w("BITMAP-COMPRESS", "未处理：" + data.length);
        BitmapFactory.Options options = new BitmapFactory.Options();
        //true那么将不返回实际的bitmap对象,不给其分配内存空间但是可以得到一些解码边界信息即图片大小等信息
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeByteArray(data, 0, data.length, options);
        //计算最佳压缩比例 inSampleSize
...
//      下面进行质量压缩
        ...
        Log.w("BITMAP-COMPRESS", "大小压缩/质量压缩前：" + bytes.length);


//        int qua = 100;
////        final int M = 1024 * 1024;
//        if (bytes.length > quality) {
//            qua = (int) (100f * quality / bytes.length);
//        }
//        bos.reset();
//        bitmap.compress(Bitmap.CompressFormat.JPEG, qua, bos);

        int qua = 100;
        while (bos.toByteArray().length > quality) {  //循环判断如果压缩后图片是否大于100kb,大于继续压缩
            bos.reset();//重置baos即清空baos
            //第一个参数 ：图片格式 ，第二个参数： 图片质量，100为最高，0为最差  ，第三个参数：保存压缩后的数据的流
            bitmap.compress(Bitmap.CompressFormat.JPEG, qua, bos);//这里压缩options%，把压缩后的数据存放到baos中
            qua -= 10;//每次都减少10
        }

        Log.w("BITMAP-COMPRESS", "质量压缩后：" + bos.toByteArray().length);
...
//      如果 inPurgeable 设为True的话表示使用BitmapFactory创建的Bitmap, 用于存储Pixel的内存空间在系统内存不足时可以被回收
        ...
        //inPurgeable和inInputShareable 这两个需要一起使用，BitmapFactory.java的源码里面有注释，大致意思是表示在系统内存不足时是否可以回收这个bitmap，
        // 有点类似软引用，但是实际在5.0以后这两个属性已经被忽略，
        // 因为系统认为回收后再解码实际会反而可能导致性能问题
     ...
     */
    private static BitmapFactory.Options calculateInSampleSize(final BitmapFactory.Options options, final int reqWidth, final int reqHeight) {
        // 源图片的高度和宽度
        final int OUTHEIGHT = options.outHeight;
        final int OUTWIDTH = options.outWidth;
        Log.w("BITMAP-COMPRESS", "OUTWIDTH:OUTHEIGHT = " + OUTWIDTH + ":" + OUTHEIGHT);

        int inSampleSize = 1;
        if (OUTHEIGHT > 400 || OUTWIDTH > 450) {
